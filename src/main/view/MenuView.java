...
public class MenuView {
	...
	private String currnetUserId = null;
	...
	public void displayMenu() throws Exception {
		while (true) {
			...
			case "1":
				...
				break;
			case "2":
				LoginResultDto lrd = handleSignOn();
				if(lrd.isSuccess()) {					
					stateView(lrd.isSuccess(), "login");
					OrderView ov = new OrderView(currnetUserId);
	...
	// 로그인
	private LoginResultDto handleSignOn() {
		System.out.print("로그인 - 아이디 입력: ");
		...
		LoginResultDto lrd = uc.Login(id, pw);
		
		if (lrd.isSuccess()) {
			currentUser = lrd.getName();
			currnetUserId = id;
			return lrd;
		} else {
			return lrd;
